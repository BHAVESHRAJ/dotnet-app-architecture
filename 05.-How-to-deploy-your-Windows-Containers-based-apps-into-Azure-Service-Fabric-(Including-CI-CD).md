...
**IMPORTANT** (VM size/memory): If you are deploying just the MVC app container using mock-data or a remote SQL Server (like Azure SQL Database), you can select a D1_V2 Standard VM size which doesn't have much memory and it is cheap.
 ![image](https://user-images.githubusercontent.com/1712635/30514707-3c54c786-9acf-11e7-824d-e6c10139dece.png)
However, **if you will also want to deploy a SQL Server container** (the other SF app definition) as part of the same cluster, **then you will need a larger VM, with more memory (at least 7GB memory), like the D2_V2 or D3_V2 size**.
