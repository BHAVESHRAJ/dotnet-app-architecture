...
This post describes how to deploy eShopModernizedMVC to Service Fabric, but deploying the eShopModernizedWebForms should be pretty similar, so you can use almost the same procedure but using different deployment .yml files.
...
![image](https://user-images.githubusercontent.com/1712635/31355877-cd65c648-acf0-11e7-930a-169e1873484f.png)
...
- Access to [eShopModernizedMVC image](https://hub.docker.com/r/eshop/modernizedmvc/) at [Docker Hub](https://hub.docker.com/) (or any other Docker Registry like Azure Container Registry if you push/publish your own image)
...
If you choose to deploy your own eShopModernizedMVC or eShopModernizedWebForms custom Docker images, it is important that you must, in the first place, have tested either the eShopModernizedMVC or eShopModernizedWebForms container in a local Docker for Windows environment so it is now ready to be built and packaged in a Service Fabric application.

If you choose to deploy our ready-to-deploy our [eShopModernizedMVC](https://hub.docker.com/r/eshop/modernizedmvc/) or [eShopModernizedWebForms](https://hub.docker.com/r/eshop/modernizedwebforms/) custom Docker images, you just need to follow the procedure related to Azure Service Fabric, below.
...
5. Select **Node type 1 (Primary)** and fill out the **Node type configuration** form. Enter a node type name (like "d1v2type" which is based on the type of the VM to be selected)  and set the [Durability tier](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-cluster-capacity#the-durability-characteristics-of-the-cluster) to "**Bronze**." Select a VM size.
...
For any production deployment, [capacity planning](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-cluster-capacity) is an important step. 

**NOTE** (VM size/memory): If you are deploying just the MVC app container using mock-data and the SQL Server container the D1_V2 Standard VM might be enough (as tested by us). But if deploying many more large containers like the SQL Container, you might need more memory in the VM size like the D2_V2 or D3_V2 size.
But, initially, the D1_V2 Standard VM shuld be enough.
 ![image](https://user-images.githubusercontent.com/1712635/30514707-3c54c786-9acf-11e7-824d-e6c10139dece.png)

Select "Silver" for the [reliability tier](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-cluster-capacity#the-reliability-characteristics-of-the-cluster) and an initial virtual machine scale set **capacity of 5** VMs if you want to see how the containers are distributed across the cluster.
Just for testing, you could also create a single VM/instance which cannot have high availability but will be cheaper and might be enough for quick tests. 
...
![image](https://user-images.githubusercontent.com/1712635/30515271-53ba04b8-9ad9-11e7-9f33-a52629adbc5d.png)

Then, in **Connection Endpoint**, select/enter the management endpoint for the cluster. For example, `eshopmodernizedcluster.westus.cloudapp.azure.com:19000`. You can find the client connection endpoint in the Overview blade for your cluster in the Azure portal.

![image](https://user-images.githubusercontent.com/1712635/30515279-960b96e2-9ad9-11e7-9e80-89f0b9e082d2.png)

Click **Publish**.

IMPORTANT NOTE: The first time you deploy a container type into SF plus the eShopModernized app it might take sometime (just a few minutes) until you can run the app properly from the browser. 
First, the nodes need to pull the Windows and ASP.NET images. Second, the application will populate sample data into the database, the first time, which also takes some time.
...
![image](https://user-images.githubusercontent.com/1712635/30515328-3d880c48-9ada-11e7-85db-aaf9d43eba34.png)

Note that if you connect to SQL Server and change data by using the SQL Server manager, that should have changed the data in the application if mockdata=False in the env variables.
...
Or you can just deploy the Service Fabric app metadata-manifest project already available at the repo, in the same way you deployed the MVC container:
https://github.com/dotnet-architecture/eShopModernizing/tree/master/ServiceFabric/eShopModernizedSQL_ServiceFabricApp
...
As a pre-requisite we need the images pushed to our container registry, as we have shown in the [section 3 of the wiki about the CI CD for Azure VMs](https://github.com/dotnet-architecture/eShopModernizing/wiki/03.-How-to-deploy-your-Windows-Containers-based-app-into-Azure-VMs-(Including-CI-CD)#scenario-c-deploying-from-vsts-cd-pipeline-to-an-azure-vm-using-the-docker-tasks-in-vsts) Our pipeline will take as artifact the github repository, where we have the Kubernetes configuration files. 

![CD pipeline](https://github.com/dotnet-architecture/eShopModernizing/blob/master/img/cicdservicefabric/CDPipeline.PNG)


![CD artifact](https://github.com/dotnet-architecture/eShopModernizing/blob/master/img/cicdservicefabric/CDArtifact.PNG)

The tasks of the release deploy the services. The first step is to replace the tokens. This is an optional step to update the environment variables using variables of the Release. This task will be explained at the end of this section. The first mandatory step is to deploy the MVC application. We use the 'Service Fabric Application Deployment' task. We have to pass here the Application package of our Service Fabric project for the service, and the 'Publish Profile' for the variables.

![Deploy SQL](https://github.com/dotnet-architecture/eShopModernizing/blob/master/img/cicdservicefabric/DeployMVC.PNG)

The deployment of the SQL container is equal to the previous task but pointing to the SQL Servide Fabric project files.

![Deploy MVC](https://github.com/dotnet-architecture/eShopModernizing/blob/master/img/cicdservicefabric/DeploySQL.PNG)

In the case of the CD for the WebForms application we repeat the previous process, but changing the MVC application by the Webforms one.

![Deploy WebForms](https://github.com/dotnet-architecture/eShopModernizing/blob/master/img/cicdservicefabric/DeployWebForms.PNG)

Yo can see that we have disabled the SQL deployment task for the WebForms CD. In case that you want to launh the CD for MVC and Webforms to the same Service Fabric Cluster you will not want to deploy twice the SQL, so it has sense to disable this step in one of your CD pipelines. Otherwise the SQL deployment should be enabled. 

### Updating environment variables with VSTS variables 

With Service Fabric, we can define the variables that will change for each deployment and store them in the Publish Profile. In this case we have used the Cloud.xml file inside the 'eShopLegacyWebApps\ServiceFabric\eShopModernizedMVC-ServiceFabricApp\ApplicationParameters' folder. There we have put for instance the variable to store the Application Insights Instrument Key.

```
<?xml version="1.0" encoding="utf-8"?>
<Application Name="fabric:/eShopModernizedMVC_ServiceFabricApp" xmlns="http://schemas.microsoft.com/2011/01/fabric" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <Parameters>
    <Parameters>
    <Parameter Name="WebForms_Container_InstanceCount" Value="-1" />    
    <Parameter Name="UseAzureStorage" Value="#{use_azure_storage}#"/>
    <Parameter Name="StorageConnectionString" Value="#{your_storage_connection_string}#"/>
    <Parameter Name="InstrumentationKey" Value="#{your_app_insights_instrument_key}#" />
    <Parameter Name="UseAzureActiveDirectory" Value="#{use_azure_active_directory}#" />
    <Parameter Name="AzureActiveDirectoryClientId" Value="#{your_aad_client_id}#" />
    <Parameter Name="AzureActiveDirectoryTenant" Value="#{your_aad_directory_tenant}#" />
    <Parameter Name="PostLogoutRedirectUri" Value="#{your_aad_logout_refirect_uri}#" />
  </Parameters>
</Application>
```
...
We will set this value in the release process, using the 'Replace Tokens' task. We have set the task to take all the xml files and replace the pattern '{key_name}#' by the key_name if this one is found as a variable in the release.
 ...
 ![Replace Tokens](https://github.com/dotnet-architecture/eShopModernizing/blob/master/img/cicdservicefabric/ReplaceTokens.PNG)

In our example we have added these keys to the variables in the CD. As you can see the name of the variables corresponds to the replace tokens in the previous Service Fabric configuration file.

 ![Environment variables](https://github.com/dotnet-architecture/eShopModernizing/blob/master/img/cicdservicefabric/EnvironmentVariables.PNG)
...
- Create secure Service Fabric cluster quickly with no explicit ARM templates
    [http://blog.randomdust.com/2017/05/new-service-fabric-powershell-cmdlets/](http://blog.randomdust.com/2017/05/new-service-fabric-powershell-cmdlets/)
...
[https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-dnsservice](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-dnsservice)
