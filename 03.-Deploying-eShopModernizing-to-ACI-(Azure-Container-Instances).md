...
At this point you should be able to run and see the MVC appliation in a browser, like here:
...
Since the app is using the connection string pointing to the SQL Server container, the app creates the database the first time it runs and the database still doesn't exist by using code in the app based on **Entity Framework Migrations**. That's why the first execution of the app takes longer.
Once the app has the database created in the SQL Container any data operation like querying and persisting the data is working against the database in that SQL Server container.
