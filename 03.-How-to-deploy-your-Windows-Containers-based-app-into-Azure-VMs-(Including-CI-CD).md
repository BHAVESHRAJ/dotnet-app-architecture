...
2. Add or modify the hosts settings and append tcp://0.0.0.0:2375 (un-secured port) and tcp://0.0.0.0:2376 (secured port), like in the following JSON snippet.
    
    BOTH PORTS plus internal npipe

     `{
       "hosts": ["tcp://0.0.0.0:2375","tcp://0.0.0.0:2376","npipe://"]
     }`

    ONLY UN-SECURED PORT plus internal npipe
...
This configuration will listen for all IP addresses on the specified ports (2375 and/or 2376 ports). 
...
If you're creating the certs with an IP address, you may want to consider a static IP, or even better, use a DNS name to avoid having to recreate certs when the IP address changes.
