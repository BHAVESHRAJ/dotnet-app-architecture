...
The tasks of the Release pipeline basically will do four actions: remove old containers, remove old images, deploy new images and run the containers.
...
#### 3. Remove SQL image
Exactly as the previous task, but for the SQL image. This step is not needed if you are not using one SQL database in a container.
